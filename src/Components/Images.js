...
const Images = () => {
  ...
  return (
    <div className='images'>
         ...
         <button type="button" className="crawl_btn" onClick={handleClassify}>Classify</button>
          ...
          {classifiedData.appropriate.map((url) => (
            <li key={url} className="list_styling image_styling">
              ...
              <div className="audit_buttons">
                  <button onClick={() => {handlePass(url); passCount();}} className='auditbtn1'>Pass</button>
                  <button onClick={() => {handleOff(url); offCount();}} className='auditbtn2'>Off</button>
                  <button onClick={() => handleSkip(url)} className='auditbtn3'>Skip</button>
                </div>
            </li>
          ))}</div>
